...
/**
 * @file Engine.hpp
 * @brief P3D Game Engine — core EC/DOD hybrid framework.
 *
 * Implements the architecture described in "The P3D Project: Game Engine
 * HLD" (Entities, Components, Systems, Managers, and the Core Engine Loop).
 *
 ...
 *       separate header. See the companion example for a reference game
 *       module built on top of this file.
 *
 * @note Requires C++17 (inline variables are used to keep this a true
 *       single-header library with no companion .cpp).
 */
 ...
 *
 * There is no FPU on the NDS, so all game-logic math (positions, damage
 * rolls, timers, delta-time, etc.) must go through this type instead of
 * float/double.
 */
...
//
// Per the HLD's "no dynamic allocation in the core loop" requirement, every
// container in this file is fixed-capacity (etl::vector / etl::pool) rather
// than std::vector/new/delete. These are only *defaults*: `Engine` is a
 ...
 *        communication, per the HLD's `etl::message_bus` design.
 *
 * Declared `inline` so this header can be included in multiple translation
 * units without needing a companion .cpp (single-header library goal).
 */
 ...
 *        singleton accessor.
 *
 * Each concrete System/Manager subclass (e.g. `BattleSystem`,
 * `RenderManager`) gets its own independent singleton instance by inheriting
 * `Singleton<TDerived>` with itself as `TDerived` — there is deliberately no
 * single shared "System instance" at the abstract-base level, since a game
 * has many distinct System/Manager subclasses.
 *
 ...
 *
 * @note Ownership: per the HLD's memory design, Components are allocated
 *       from a shared pool owned by `Engine` (not `new`/`delete`). Entity
 *       only stores pointers into that pool; it does not own the memory.
 */
class Entity
{
  public:
     ...
     *        and calls its `Init()`.
     * @param c Pointer to a Component allocated by `Engine` (not owned).
     */
...
class Component
{
  ...
  protected:
     ...
     * per component" without type erasure, which we want to avoid on this
     * hardware.
     */
 ...
 * functions, per the HLD's HAL design.
 *
 ...
 *       the singleton behavior described in the HLD.
 */
class Manager
{
  public:
 ...
 * Components, or message types — those are all registered/created by game
 * code at runtime. This keeps the engine itself fully hardware- and
 * game-agnostic, per the HLD's portability goal.
 *
...
class Engine
{
  public:
     ...
     *        hardcoded) to keep `Engine` hardware-agnostic, per the HAL
     *        design — only Managers may touch hardware directly, and the
     *        function supplied here is expected to itself delegate to a
     *        Manager.
     */
