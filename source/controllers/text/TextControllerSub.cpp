...
TextControllerSub::~TextControllerSub()
{
    ...
    dmaFillHalfWords(0, videoBuffer, 256 * 256 * sizeof(uint16_t));
    free(fontBitmap);
    free(fontPalette);
    //free(fontGlyphs);
...
void* TextControllerSub::openFile(const std::string& path)
{
...
bool TextControllerSub::loadFontBitmap(const std::string& path)
{
...
bool TextControllerSub::loadFontPalette(const std::string& path)
{
    void* buffer = openFile(path);
    if (buffer == nullptr)
    {
    ...
    dmaCopy(fontPalette, BG_PALETTE_SUB, 256 * sizeof(uint16_t)); // Copy the palette to the background palette memory
    return true;
}

bool TextControllerSub::loadFontMetadata(const std::string& path)
{
    u32 size;
...
void TextControllerSub::drawText(const std::string& text, int startX, int startY, int color)
{
    int cursorX = startX;
    int cursorY = startY;
...
void TextControllerSub::drawGlyph(const Glyph& glyph, int cursorX, int cursorY, int color)
{
...
void TextControllerSub::drawPixel(int x, int y, int paletteIndex)
{
...
int TextControllerSub::extractIntValue(const std::string& line, const std::string& key)
{
...
void TextControllerSub::haltOnError(const std::string& errorMessage)
{
    consoleDemoInit();
    iprintf("Error: %s\n", errorMessage.c_str());
    while (1)
...
void TextControllerSub::testBitmap()
{
    for (int y = 0; y < 256; y++)
    {
        for (int x = 0; x < 256; x++)
...
void TextControllerSub::testPalette()
{
