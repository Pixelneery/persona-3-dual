...
std::vector<SpriteDBEntry> SPRITE_DB;
int SPRITE_DB_LEN = 0;

static std::vector<GritAsset> spriteAssets;

static void addSpriteEntry(SpriteType type, int spriteId, const char* spriteName)
{
...
void initializeSpriteDatabase()
{
    ...
    SPRITE_DB_LEN = static_cast<int>(SPRITE_DB.size());
}

void unloadSpriteDatabase()
{
    for (size_t i = 0; i < spriteAssets.size(); ++i)
    {
        graphicsCtrl.unloadGrit(spriteAssets[i]);
    }
