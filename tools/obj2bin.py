...
# Helper to force signed 16-bit values for C++ output to prevent narrowing conversion errors
def to_s16(val):
    val = int(val) & 0xFFFF
...
def convert_anim_json(input_file, output_file, tex_width=None, tex_height=None, vertex_color=None):
    ...
    with open(header_out, 'w') as out:
        ...
        # 2. Compile Keyframes (Dividing positions by 16.0 to fix scale!)
        ...
        # 3. LOADER FUNCTION (Now injects pivot points)
        ...
        for node in data['nodes']:
            nid = node['id']
            pid = node['parent']
            # Divide origin by 16.0 to match the geometry scale
