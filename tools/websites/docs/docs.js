...
async function loadIndex() {
    ...
    try {
        ...
        renderError('Could not load docs manifest.', `Make sure <code>manifest.json</code> exists in the docs folder. (${err.message})`);
        return;
    }
...
async function loadDoc(file) {
    ...
    if (!/^[\w][\w\-./ ]*\.md$/i.test(file) || file.includes('..')) {
        renderError('Invalid document path.', 'The requested file path is not allowed.');
        return;
    }
